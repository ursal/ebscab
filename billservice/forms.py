...
class SystemUserForm(ModelForm):
    ...
    text_password = forms.CharField(label = _(u"Пароль") if  settings.HIDE_SUPERUSER_PASSWORDS==False else _(u"Изменить пароль"), required=False, widget = CustomPasswordWidget()  if  settings.HIDE_SUPERUSER_PASSWORDS==True else forms.widgets.TextInput())
    class Meta:
        model = SystemUser
...
class SendSmsForm(forms.Form):
    ...
    template = forms.ChoiceField(label=_(u'Шаблон'), choices = [], required=False, widget = forms.widgets.Select(attrs={'rows':4, 'class': 'input-large span5'}))
    body = forms.CharField(label=_(u'Сообщение'), widget = forms.widgets.Textarea(attrs={'id': 'id_sendsms_body', 'rows':4, 'class': 'input-large span5'}), help_text=_(u"Можно использовать {{account.ballance}}, {{account.fullname}}, {{account.username}}, {{account.contract}}"))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['template'].choices=[['', '---']]+list(Template.objects.filter(type__id=11).values_list('body', 'name', ))
